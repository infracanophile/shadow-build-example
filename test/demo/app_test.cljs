(ns demo.app-test
  ...
  (:require [cljs.test :as t]
            ...
            [demo.clara-rules-testfacts :refer [->Temperature Temperature
                                                ->WindSpeed WindSpeed
...
(defsession my-session 'demo.app-test)
(defsession my-session-map 'demo.app-test :fact-type-fn :type)
(defsession my-session-data (demo.clara-test-rules-data/weather-rules))
...
(deftest test-throw-rhs
  (try
    ...
    (catch :default e
      ...
      (is (= {:?t 999999}
             (:bindings (ex-data e))))
      (is (= "demo.app-test/throw-on-bad-temp"
             (:name (ex-data e)))))))
